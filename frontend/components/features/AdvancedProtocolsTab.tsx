                ...
                <tr key={p.id} className={`hover:bg-gray-50 dark:hover:bg-gray-900/50 transition cursor-pointer ${editing ? "ring-2 ring-inset ring-blue-500/50" : ""}`} onClick={() => !editing && toggleExpand(p.id)}>
                  {editing ? (
                    ...
                    <>
                      ...
                      {td(<div className="flex items-center gap-1.5"><span className="font-mono text-xs text-gray-900 dark:text-gray-100">{p.api_path}</span>{p.description && <span className="relative group shrink-0"><span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-gray-200 dark:bg-gray-700 text-[10px] font-bold text-gray-500 dark:text-gray-400 group-hover:bg-blue-100 group-hover:text-blue-600 dark:group-hover:bg-blue-900/40 dark:group-hover:text-blue-400 leading-none select-none cursor-help transition-colors">?</span><span className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 px-3.5 py-2.5 bg-gray-800 dark:bg-gray-100 text-white dark:text-gray-900 text-[13px] rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible pointer-events-none z-50 max-w-[240px] text-left whitespace-normal leading-relaxed transition-opacity">{p.description}</span></span>}</div>)}
