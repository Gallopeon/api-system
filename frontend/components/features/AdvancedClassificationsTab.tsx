...
export default function AdvancedClassificationsTab({ classifications, busy, createClassification, updateClassification, deleteClassification, notifyError, t }: Props) {
    ...
    <div className="space-y-4">
      ...
      {classifications.length === 0 ? (
        ...
        <div className={`${cardClass} p-0 overflow-hidden`}>
          <table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-900 border-b dark:border-gray-800"><tr>{[t("API Path", "API 路径"), t("Category", "类别"), t("PII", "PII"), t("GDPR", "GDPR"), t("Retention", "保留"), t("Notes", "备注"), t("Actions", "操作")].map(h => th(h))}</tr></thead>
            ...
            {classifications.map(c => {
              ...
              return (
                <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition">
                  {editId === c.id ? (
                    <>{td(<input className={inputClass} value={ePath} onChange={e => setEPath(e.target.value)} />)}{td(<select className={inputClass} value={eCat} onChange={e => setECat(e.target.value)}><option value="public">{t("Public", "公共")}</option><option value="internal">{t("Internal", "内部")}</option><option value="confidential">{t("Confidential", "机密")}</option><option value="pii">{t("PII", "个人信息")}</option></select>)}{td(<input type="checkbox" checked={ePii} onChange={e => setEPii(e.target.checked)} />)}{td(<input type="checkbox" checked={eGdpr} onChange={e => setEGdpr(e.target.checked)} />)}{td(<input className={inputClass} type="number" value={eRet} onChange={e => setERet(e.target.value)} />)}{td(<input className={inputClass} value={eNote} onChange={e => setENote(e.target.value)} />)}{td(<div className="flex gap-1"><button className="p-1.5 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg" onClick={saveEdit}><Check className="w-3.5 h-3.5" /></button><button className="p-1.5 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg" onClick={() => setEditId(null)}><X className="w-3.5 h-3.5" /></button></div>)}</>
                  ) : (
                    <>{td(<span className="font-mono text-xs text-gray-900 dark:text-gray-100">{c.api_path}</span>)}{td(<span className={`text-xs px-2 py-0.5 rounded-full font-medium ${catStyle}`}>{c.data_category}</span>)}{td(<span className={c.contains_pii ? "text-red-600 dark:text-red-400 font-semibold" : "text-gray-400"}>{c.contains_pii ? t("Yes", "是") : t("No", "否")}</span>)}{td(<span className={c.gdpr_relevant ? "text-orange-600 dark:text-orange-400 font-semibold" : "text-gray-400"}>{c.gdpr_relevant ? t("Yes", "是") : t("No", "否")}</span>)}{td(<span className="text-gray-600 dark:text-gray-400">{c.retention_days}d</span>)}{td(<span className="text-gray-500 text-xs max-w-[120px] truncate block">{c.notes || "—"}</span>)}{td(<div className="flex items-center gap-1"><button className="p-1.5 rounded-lg text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition" onClick={() => startEdit(c)}><Edit3 className="w-3.5 h-3.5" /></button><button className="p-1.5 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition" onClick={() => deleteClassification(c.id)} disabled={busy}><Trash2 className="w-3.5 h-3.5" /></button></div>)}</>
                  )}
                </tr>
              );
            })}
